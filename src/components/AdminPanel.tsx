...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onBackToSite }) => {
  ...
  // Authentication State. Firebase verifies this session on every page load.
  ...
  useEffect(() => {
    if (!auth) {
      setAuthError('Yönetici girişi şu anda yapılandırılamadı. Firebase ayarlarını kontrol edin.');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!auth) {
      setAuthError('Yönetici girişi şu anda kullanılamıyor.');
      return;
    }
    ...
    try {
      ...
      setAuthError('E-posta veya şifre hatalı. Lütfen tekrar deneyin.');
    } finally {
  ...
  if (isCheckingSession) {
    return (
      <div className="min-h-screen bg-zinc-900 text-white flex items-center justify-center p-4">
        <p className="text-sm font-bold text-zinc-300">Yönetici oturumu kontrol ediliyor...</p>
      </div>
    );
  }
