...
export const CartDrawer: React.FC<CartDrawerProps> = ({
  ...
  const getCartItemUnitPrice = (cartItem: (typeof cart)[number]) => {
    let price = cartItem.item.price || 0;
    if (cartItem.item.category === 'pizzalar') {
      if (cartItem.selectedSize === 'Küçük') price = Math.round(price * 0.8);
      if (cartItem.selectedSize === 'Büyük') price = Math.round(price * 1.3);
    }
    return price;
  };
  ...
  const handleWhatsAppOrder = (e: React.FormEvent) => {
    ...
    const itemsText = cart
      .map((c) => {
        ...
        const lineTotal = getCartItemUnitPrice(c) * c.quantity;
        return `• ${c.item.name}${sizeStr} x${c.quantity} — ${formatCurrency(lineTotal)}${noteStr}`;
      })
      .join('\n');

    let text = `*${restaurantInfo.name.toLocaleUpperCase('tr-TR')} - YENİ SİPARİŞ*\n\n`;
    text += `*Sipariş Özeti:*\n${itemsText}\n\n`;
    text += `*Ürün Toplamı:* ${formatCurrency(totalAmount)}\n`;
    text += `*Teslimat Ücreti:* Ücretsiz\n`;
    text += `*Tahmini Toplam:* ${formatCurrency(totalAmount)}\n\n`;
    ...
    text += `\n_Lütfen siparişi ve teslimat süresini onaylayınız._`;
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-between mt-2">
                          <span className="text-[11px] text-zinc-500 font-medium italic">
                            {formatCurrency(getCartItemUnitPrice(cartItem) * cartItem.quantity)}
                          </span>
              ...
              <form onSubmit={handleWhatsAppOrder} className="pt-4 border-t border-zinc-200 space-y-3">
                ...
                {orderSentSuccess && (
                  <div className="p-3 bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs rounded-xl flex items-center gap-2 font-bold">
                    ...
                    <span>WhatsApp açıldı. Gönder'e basınca siparişiniz işletmeye ulaşır.</span>
                  </div>
                )}
